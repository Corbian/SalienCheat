...
Msg( 'IgnoreUpdates is ' . number_format( !$UpdateCheck ) . ' - DisableColors is ' . number_format( $DisableColors ) );
...
do
{
	...
	if( empty( $Data[ 'response' ][ 'new_score' ] ) )
	{
		...
		{
			$LagAdjustedWaitTime = max( $FailSleep, min( $ScanPlanetsTime, $SkippedLagTime ) );
		}
